...
function setCardState(replay, uid, state) {
...
async function batchReq(env, body, method = "POST") {
...
describe("handleCardBatchAction", () => {
  ...
  it("terminates active cards", async () => {
    ...
    setCardState(env.CARD_REPLAY, "ff000000000002", "active");
    ...
    expect(json.results.every(r => r.status === "terminated")).toBe(true);

    const s1 = env.CARD_REPLAY.__cardStates.get("ff000000000001");
    expect(s1.state).toBe("terminated");
    const s2 = env.CARD_REPLAY.__cardStates.get("ff000000000002");
    expect(s2.state).toBe("terminated");
  });

  it("skips already terminated cards", async () => {
    const env = makeEnv();
    setCardState(env.CARD_REPLAY, "ff000000000001", "terminated");
  ...
  it("activates keys_delivered cards", async () => {
    const env = makeEnv();
    setCardState(env.CARD_REPLAY, "ff000000000001", "keys_delivered");
    const resp = await batchReq(env, { uids: ["ff000000000001"], action: "activate" });
    expect(resp.status).toBe(200);
    const json = await resp.json();
    expect(json.results[0].status).toBe("activated");
    const s = env.CARD_REPLAY.__cardStates.get("ff000000000001");
    expect(s.state).toBe("active");
  });

  it("activates discovered cards", async () => {
    const env = makeEnv();
    setCardState(env.CARD_REPLAY, "ff000000000001", "discovered");
    const resp = await batchReq(env, { uids: ["ff000000000001"], action: "activate" });
    const json = await resp.json();
    expect(json.results[0].status).toBe("activated");
  });
  ...
  it("handles mixed results across multiple cards", async () => {
    const env = makeEnv();
    setCardState(env.CARD_REPLAY, "ff000000000001", "active");
    setCardState(env.CARD_REPLAY, "ff000000000002", "terminated");
    setCardState(env.CARD_REPLAY, "ff000000000003", "active");
    const resp = await batchReq(env, {
    ...
    const statuses = json.results.map(r => r.status);
  ...
  it("returns errors array on DO failure", async () => {
    const env = makeEnv();
    env.CARD_REPLAY.__cardStates.set("ff000000000001".toLowerCase(), {
      state: "new",
