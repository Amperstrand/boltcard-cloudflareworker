...
describe("BoltCard Crypto Tests", () => {
  ...
  TEST_VECTORS.forEach(({ p, c, k1, k2, expected_uid, expected_ctr, expected_sv2, expected_ks, expected_cm, expected_ct }) => {
    test(`Decoding p=${p} and c=${c}`, () => {
      ...
      const decryption = decryptP(bytesToHex(pBytes), [k1Bytes]);

      expect(decryption.success).toBe(true);
      if (!decryption.success) return;
      expect(bytesToHex(decryption.uidBytes)).toBe(expected_uid);
      expect(bytesToHex(decryption.ctr)).toBe(expected_ctr);

      const { sv2, ks, cm, ct } = buildVerificationData(decryption.uidBytes, decryption.ctr, k2Bytes);
...
test("_xorArrays should correctly XOR two byte arrays", () => {
  ...
  expect(_xorArrays(a, b)).toEqual(new Uint8Array([5, 7, 5]));
});
...
test("_shiftGo should correctly shift bytes left", () => {
  const input = new Uint8Array([0b01000000, 0b00000001]);
  const { shifted, carry } = _shiftGo(input);
  expect(shifted).toEqual(new Uint8Array([0b10000000, 0b00000010]));
  expect(carry).toBe(0);
});
...
test("computeAesCmac should generate expected CMAC", () => {
  ...
  expect(bytesToHex(cmac).length).toBe(32);
});
...
test("decryptP should work with single K1 key (existing behavior)", () => {
  ...
  const k1Bytes = hexToBytes(k1);
  const decryption = decryptP(p, [k1Bytes]);
...
test("verifyCmac returns cmac_validated=true when CMAC matches", () => {
  ...
  expect(decryption.success).toBe(true);
  if (!decryption.success) return;
  const result = verifyCmac(decryption.uidBytes, decryption.ctr, c, k2Bytes);
  expect(result.cmac_validated).toBe(true);
...
describe("BTCPayServer.BoltCardTools-inspired tests", () => {
  ...
  test("CanDecryptSunPICCData — p=4E2E..., uid=04996c6a926980, ctr=3", () => {
    const p = "4E2E289D945A66BB13377A728884E867";
    const decryption = decryptP(p, [k1]);
    expect(decryption.success).toBe(true);
    if (!decryption.success) return;
    expect(bytesToHex(decryption.uidBytes)).toBe("04996c6a926980");
    expect(bytesToHex(decryption.ctr)).toBe("000003");
  });
  ...
  test("CanDecryptSunPICCData — p=00F4..., uid=04996c6a926980, ctr=5", () => {
    ...
    if (!decryption.success) return;
    expect(bytesToHex(decryption.uidBytes)).toBe("04996c6a926980");
    expect(bytesToHex(decryption.ctr)).toBe("000005");
  });
  ...
  test("CanDecryptSunPICCData — p=0DBF..., uid=04996c6a926980, ctr=7", () => {
    ...
    if (!decryption.success) return;
    expect(bytesToHex(decryption.uidBytes)).toBe("04996c6a926980");
    expect(bytesToHex(decryption.ctr)).toBe("000007");
  });
