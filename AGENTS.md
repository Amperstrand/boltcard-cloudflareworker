...
- Nostr identity pairing (planned): cardholder links their bolt card to a Nostr npub via NIP-07 browser extension (`window.nostr.getPublicKey()`). Tap card → server links UID → npub in KV (`card_nostr:<uid>`). Future taps include Nostr claims in VCs. Server can optionally fetch kind 0 metadata and NIP-85 assertions from relays via WebSocket (`fetch` with upgrade). Uses existing `@noble/secp256k1` for Schnorr verification and `@scure/base` for bech32 npub decoding.
