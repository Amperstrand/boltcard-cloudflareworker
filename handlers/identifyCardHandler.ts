import { extractUIDAndCounter, validateCmac, type ExtractResult } from "../boltCardHelper.js";
import type { CardConfig, CardStateRow } from "../types/core.js";
...
export async function handleIdentifyCard(request: Request, env: Env): Promise<Response> {
  ...
  const { uidHex, ctr } = decryption as { uidHex: string; ctr: string };
  ...
  if (cardState && cardState.state !== CARD_STATE.TERMINATED) {
    ...
    let config: CardConfig | null | undefined;
    try {
    ...
    if (config && config.K2) {
      const cmac = validateCmac(uidBytes, ctrBytes, cHex, hexToBytes(config.K2 as string));
      results.push({
        source: "config",
  ...
  const keyCache = new Map<number, { k0: string; k1: string; k2: string; k3: string; k4: string; cardKey: string; id: string }>();
  let detAttempts: Array<{ version: number; cmac_validated: boolean }> = [];
  try {
    const scanResult: { matchedVersion: number | null; attempts: Array<{ version: number; cmac_validated: boolean }> } = await cmacScanVersions(uidBytes, ctrBytes, cHex, {
  ...
  const match = results.find((r: Record<string, unknown>) => r.cmac_validated);
